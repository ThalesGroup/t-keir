...
def process_file(args, host, port, input_file):
    ...
    try:
        with open(input_file, encoding="utf-8") as input_f:
            ...
            input_f.close()
        r = requests.post(
            args.scheme + "://" + host + ":" + str(port) + "/api/syntactictagger/run",
            json=json_request,
            verify=args.no_ssl_verify,
        )
        if r.status_code == 200:
            filename = os.path.basename(input_file).replace(".json", ".syntax.json")
...
def main(args):
    ...
    try:
        # initialize logger
        syntacticConfiguration = SyntacticTaggerConfiguration()
