...
def process_file(args, host, port, input_file):
    ...
    try:
        with open(input_file, encoding="utf-8") as input_f:
            ...
            input_f.close()
        r = requests.post(
            args.scheme + "://" + host + ":" + str(port) + "/api/mstagger/run", json=json_request, verify=args.no_ssl_verify
        )
        if r.status_code == 200:
            filename = os.path.basename(input_file).replace(".json", ".ms.json")
...
def main():
    ...
    try:
        # initialize logger
        morphosyntacticConfiguration = MorphoSyntacticTaggerConfiguration()
        with open(args.config) as fh:
            morphosyntacticConfiguration.load(fh)
            fh.close()
        ThotLogger.loads(morphosyntacticConfiguration.logger_config.configuration)
        host = morphosyntacticConfiguration.net_config.configuration["network"]["host"]
        port = morphosyntacticConfiguration.net_config.configuration["network"]["port"]
        ThotLogger.info("MS:" + args.input)
