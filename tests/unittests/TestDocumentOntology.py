...
class TestOntologyBuilder:
    ...
    def test_hypergraph_scopes_spo_under_chunks_with_intersection(self):
        """Document → chunk sub-ontologies share concepts across chunks."""
        ...
        document["golden_chunks"] = [
            ...
            },
            {
                ...
                "metadata": {
                    "token_start": 4,
                    "token_end": 10,
                    "primary_entities": {"product": ["Widget Pro"]},
                    "svo_triplets": [],
                },
            },
        ]
        ...
        assert "Statement" in turtle
        assert "mentionedIn" in turtle
        assert "inChunk" in turtle
        assert "SubOntology" in turtle
        assert "intersectionWeight" in turtle

        chunks = list(graph.subjects(RDF.type, TKEIR.DocumentChunk))
        assert len(chunks) == 2

        # Widget Pro is evidenced in both chunk ranges → shared / support ≥ 2.
        widget = None
        for node, _p, label in graph.triples((None, None, None)):
            from rdflib.namespace import RDFS

            if _p == RDFS.label and str(label) == "Widget Pro":
                widget = node
                break
        assert widget is not None
        support = graph.value(widget, TKEIR.chunkSupport)
        assert support is not None
        assert int(support) >= 2

        # Materialized SPO remains for reasoners.
        assert "launched" in turtle.lower() or "Launched" in turtle
