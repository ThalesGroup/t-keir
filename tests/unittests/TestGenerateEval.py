...
def test_ontology_clues_sparql_from_query_terms() -> None:
    ...
    assert len(queries) == 3  # label harvest + bridge + types
    ...
    morph_queries = generate_sparql_from_query_ontology(
        morph_only, "Qui dirige Acme?"
    )
...
def test_render_report_multihop_gap() -> None:
    ...
    body = render_report([run], leaderboard=leaderboard, forge_prompt=True)
