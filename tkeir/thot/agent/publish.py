...
def publish_run(
    *,
    ...
    if approval_id:
        ...
        if approved is None:
            return {"status": "rejected", "error": "unknown approval_id"}
        ...
        if approved.correlation_id and state.correlation_id:
            if approved.correlation_id != state.correlation_id:
                return {
                    "status": "rejected",
    ...
    if approved is None:
        # Look for an existing pending/approved item for this run
    ...
    if approved is None:
        ...
        auto = os.getenv("AGENT_PUBLISH_OBSERVE_AUTO", "1") not in {
            "0",
            "false",
            "False",
        }
        ...
        approved = guard_approvals.decide(item.approval_id, status="approved")
