...
def test_generate_osint_dataset(tmp_path: pathlib.Path) -> None:
    ...
    m = json.loads(
        (tmp_path / "osint" / "manifest.json").read_text(encoding="utf-8")
    )
    assert m["count_generated"] == 20
    assert len(list((tmp_path / "osint" / "ontologies").iterdir())) >= 6
    assert (tmp_path / "osint" / "VERSION").read_text(
        encoding="utf-8"
    ).strip() == "1.1.0"
...
def test_generate_enterprise_dataset(tmp_path: pathlib.Path) -> None:
    ...
    m = json.loads(
        (tmp_path / "enterprise" / "manifest.json").read_text(encoding="utf-8")
    )
    assert m["count_generated"] == 10
    assert (tmp_path / "enterprise" / "VERSION").read_text(
        encoding="utf-8"
    ).strip() == "1.1.0"
...
def test_offline_download_safe(
    ...
    assert (tmp_path / "osint" / "ontologies" / "c2sim_core.owl").exists()
