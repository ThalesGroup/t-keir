...
def process_file(args, host, port, input_file):
    ...
    try:
        with open(input_file, encoding="utf-8") as input_f:
            ...
            input_f.close()
        r = requests.post(
            args.scheme + "://" + host + ":" + str(port) + "/api/tokenizer/run", json=json_request, verify=args.no_ssl_verify
        )
        if r.status_code == 200:
            filename = os.path.basename(input_file).replace(".json", ".tokenized.json")
...
def main():
    ...
    try:
        ...
        with open(args.config) as fh:
            tokenizerConfiguration.load(fh)
            fh.close()
        ThotLogger.loads(tokenizerConfiguration.logger_config.configuration)
        host = tokenizerConfiguration.net_config.configuration["network"]["host"]
        port = tokenizerConfiguration.net_config.configuration["network"]["port"]
