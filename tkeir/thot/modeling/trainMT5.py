...
def write_in_file(conllu_f, output_df, lang):
    for s in parse_incr(conllu_f):
        ...
        for tok in s:
            ...
            pos_sent.append(tok["upos"])
        if (len(pos_sent) > 0) and (len(form_sent) == len(pos_sent)) and (set(form_sent + pos_sent) != set(["_"])):
            ndf = pd.DataFrame(
                [
                    [
                        "lemma",
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", default=None, type=str, help="input directory containing training configuration")
