...
class SummarizerConfiguration:
    ...
    def loads(self, configuration: dict = None):
        ...
        """
        self.logger_config.loads(configuration, logger_name="summarizer")
        self.net_config.loads(configuration["summarizer"])
        self.runtime_config.loads(configuration["summarizer"])
        if "settings" in configuration["summarizer"]:
            self.configuration["settings"] = configuration["summarizer"]["settings"]
        else:
            self.configuration = {
                "settings": {"min-percent": 10, "max-percent": 20, "use-cuda": False, "language": "en", "cuda-device": 0}
            }
