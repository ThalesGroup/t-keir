...
def process_file(args, host, port, input_file):
    ...
    try:
        with open(input_file, encoding="utf-8") as input_f:
            ...
            input_f.close()
        r = requests.post(
            args.scheme + "://" + host + ":" + str(port) + "/api/zeroshotclassifier/run",
            json=json_request,
            verify=args.no_ssl_verify,
        )
        if r.status_code == 200:
            filename = os.path.basename(input_file).replace(".json", ".documentclassifier.json")
...
def main(args):
    ...
    try:
        # initialize logger
        zscConfiguration = ZeroShotClassificationConfiguration()
        with open(args.config) as fh:
            zscConfiguration.load(fh)
            fh.close()
        ThotLogger.loads(zscConfiguration.logger_config.configuration)
        host = zscConfiguration.net_config.configuration["network"]["host"]
        port = zscConfiguration.net_config.configuration["network"]["port"]
        ThotLogger.info("[ZSC]host:" + str(host))
        file_list = []
