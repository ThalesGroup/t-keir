...
def test_cli_main_full(tmp_path: Path, monkeypatch, capsys):
    ...
    with patch("thot.okf.exporter.export_full", new=fake_full):
        code = cli_main(
            ["--user-space", "dev@tkeir", "--output", str(tmp_path / "full")]
        )
    assert code == 0
...
def test_user_workspace_okf_layout(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("OKF_ROOT", raising=False)
    from thot.okf.exporter import resolve_bundle_dir, user_okf_root
    from thot.okf.store import OkfBundleStore, migrate_flat_okf_into_workspace
    from thot.tools.ingest.user_workspace import UserWorkspace

    monkeypatch.setenv("TKEIR_WORKSPACE", str(tmp_path))
    okf = user_okf_root("analyst")
    assert okf == tmp_path / "users" / "analyst" / "okf"
    assert okf.is_dir()

    bid = "bundle-user-1"
    dest = resolve_bundle_dir("analyst", bid)
    assert dest == okf / bid
    dest.mkdir(parents=True)
    (dest / "index.md").write_text("# wiki bundle\n", encoding="utf-8")
    (dest / ".tkeir-meta.json").write_text(
        json.dumps(
            {
                "bundle": {
                    "bundle_id": bid,
    ...
    found = store.get_bundle(bid, "analyst")
    assert found is not None
    assert found.user_space == "analyst"
    assert "users/analyst/okf" in found.path.replace("\\", "/")

    # Legacy flat → workspace migrate
    flat = tmp_path / "legacy-flat"
    old = flat / "legacy-b1"
    old.mkdir(parents=True)
    (old / "index.md").write_text("# old\n", encoding="utf-8")
    (old / ".tkeir-meta.json").write_text(
        json.dumps(
            {
                "bundle": {
                    "bundle_id": "legacy-b1",
    ...
    moved = migrate_flat_okf_into_workspace(flat, workspace=tmp_path)
    assert moved == ["legacy-b1"]
    assert (
        UserWorkspace("analyst", root=tmp_path).okf_dir / "legacy-b1"
    ).is_dir()
