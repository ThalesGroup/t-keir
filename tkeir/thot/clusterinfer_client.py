...
def process_file(args, host, port, input_file):
    ...
    try:
        with open(input_file, encoding="utf-8") as input_f:
            ...
            input_f.close()
        r = requests.post(
            args.scheme + "://" + host + ":" + str(port) + "/api/clusterinfer/run", json=json_request, verify=args.no_ssl_verify
        )
        if r.status_code == 200:
            filename = os.path.basename(input_file).replace(".json", ".ci.json")
            ...
            ThotLogger.error("Error :" + input_file + " connection status:"+str(r.status_code) )
            return "Error :" + input_file + " connection status:"+str(r.status_code)

        return "ok"
    except Exception as e:
        ThotLogger.error(exception_error_and_trace(str(e),str(traceback.format_exc())), trace=str(traceback.format_exc()))
        return "Error:" + str(e)
...
def main():
    ...
    try:
        # initialize logger
        relation_clusterizer_configuration = RelationClusterizerConfiguration()
        with open(args.config) as fh:
            relation_clusterizer_configuration.load(fh)
            fh.close()
        ThotLogger.loads(relation_clusterizer_configuration.logger_config.configuration)
        host = relation_clusterizer_configuration.net_config.configuration["network"]["host"]
        port = relation_clusterizer_configuration.net_config.configuration["network"]["port"]
        ThotLogger.info("HOST:" + host)
        ThotLogger.info("PORT:" + str(port))
        file_list = []
        ...
        Parallel(n_jobs=args.workers)(delayed(process_file)(args, host, port, i) for i in input_files)
