...
def process_file(args, host, port, input_file):
    ...
    try:
        with open(input_file, encoding="utf-8") as input_f:
            ...
            input_f.close()
        r = requests.post(
            args.scheme + "://" + host + ":" + str(port) + "/api/nertagger/run", json=json_request, verify=args.no_ssl_verify
        )
        if r.status_code == 200:
            filename = os.path.basename(input_file).replace(".json", ".ner.json")
...
def main(args):
    ...
    try:
        # initialize logger
        nerConfiguration = NERTaggerConfiguration()
        with open(args.config) as fh:
            nerConfiguration.load(fh)
            fh.close()
        ThotLogger.loads(nerConfiguration.logger_config.configuration)
        host = nerConfiguration.net_config.configuration["network"]["host"]
        port = nerConfiguration.net_config.configuration["network"]["port"]
        ThotLogger.info("NER:" + args.input)
        file_list = []
