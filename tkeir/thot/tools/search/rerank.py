...
async def rerank_scored_texts(
    ...
    *,
    ...
    ranked = await llm.rerank(query, documents, top_n=top_n)
...
async def rerank_vespa_children(
    ...
    *,
    ...
    Args:
        llm: Wrapper exposing ``rerank``.
        query: Query text for the cross-encoder.
    ...
    ranked = await llm.rerank(query, texts, top_n=top_n)
    ...
    LOGGER.info(
        "Reranked %d / %d Vespa hits (kept=%d)",
        len(usable),
        len(children),
        len(reordered),
    )
