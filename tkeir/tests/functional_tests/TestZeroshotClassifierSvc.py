...
from thot.zeroshotclassifier_svc import main
import os, signal
import unittest
import time
import requests
...
class TestZeroshotClassifierSvc(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        global service_pid
        if service_pid == 0:
            ...
            if service_pid == 0:
                ...
                class args:
                    config = os.path.abspath(
                        os.path.join(
                            ...
                            "../../../app/projects/default/configs/zeroshotclassifier.json",
                        )
                    )
                    init = False
                ...
                while not load_finish:
                    try:
                        r = requests.get("https://localhost:10010/api/zeroshotclassifier/health", verify=ssl_verify)
    ...
    def test_run(self):
        if service_pid:
            ...
            r = requests.post("https://localhost:10010/api/zeroshotclassifier/run", json=json_request, verify=ssl_verify)
